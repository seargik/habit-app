(() => {
  ...
  const DB_NAME = "habitAppMediaV2";
  const DB_STORE = "media";
  const OLD_DB = "habitAppPhotosV1";
  const OLD_STORE = "photos";
  const MIGRATION_FLAG = "habitApp.mediaV2Migrated";
  ...
  async function exportBackupJson() {
    ...
    const pack = { app: "Life Tracker", type: "life-tracker-media-package", version: 2, exportedAt: new Date().toISOString(), filters: f, photos: photos.map((p) => ({ ...p })) };
